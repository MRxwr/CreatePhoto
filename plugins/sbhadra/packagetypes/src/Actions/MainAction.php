...
class MainAction extends Action
{
    ...
    public function handle()
    {
        ...
        $this->addAction(Action::FRONTEND_CALL_ACTION, [$this, 'packageThemePictureTypeField']);
...
public function packageThemeField(){
  add_action('theme.reservation.exfields', function($post){
    ...
    $pAttrs='';
    if(isset($package->package_type_attributes)){
      $pAttrs = (array) json_decode($package->package_type_attributes);
    }
    //dd($pAttrs);
     $html='';
      // $packagetypes= PackageType ::where('status',1)->get();
       $packagetypes = PackageType::where('status', 1)->orderBy('odrs', 'asc')->get();
       ...
       if(!empty($packagetypes)){
          $html .='<div class="col-sm-12 pe-xl-5 pt-4">
            ...
            foreach($packagetypes as $type){
              ...
              //$packagetypeAttrs= PackageTypeAttribute ::where('package_type_id',$type->id)->where('status',1)->get();
              $packagetypeAttrs= PackageTypeAttribute ::where('package_type_id',$type->id)->where('status',1)->orderBy('odrs', 'asc')->get();
              if(!empty($attrs)){
                 $html .='<div class="set">
                      ...
                      foreach($packagetypeAttrs as $attr){
                        ...
                        if($attr->is_theme==1){
                         ...
                         if(in_array($attr->id,$attrs)){
                           ...
                           $html .='<label class="fs23 d-flex align-items-center" for="ta'.$type->id.'_'.$attr->id.'"><input class="pictype" style="margin: 5px; width: 25px; min-height: 25px;" type="radio" id="ta'.$type->id.'_'.$attr->id.'" name="pictures_type_attr_id"  value="'.$attr->id.'" '.$checked.'>'.$attr->title.'('.$attr->price.'KD)</label>';
                         }
 ...
 public function packageThemePictureTypeField(){
  add_action('theme.reservation.picturetype', function($post){
      $html='';
      if(isset($_REQUEST['theme_id']) && !empty($_REQUEST['theme_id'])){
          $themeIds = $_REQUEST['theme_id'];
          foreach($themeIds as $themeid){
             $html .= '<input type="hidden"  name="theme_id[]"  value="'.$themeid.'">';  
          }
      }
      ...
      $attrId =0;
      if(isset($_REQUEST['pictures_type_attr_id'])){
         $attrId= $_REQUEST['pictures_type_attr_id'];
      }
    $pAttrs='';
    if(isset($package->package_type_attributes)){
      $pAttrs = (array) json_decode($package->package_type_attributes);
                      ...
                      foreach($packagetypeAttrs as $attr){
                        $checked='';
                         ...
                         //if(in_array($attr->id,$attrs)){
                           ...
                           $html .='<a class="fs23" href="javascript:void(0)">'.$type->title.'</a> 
                            <div class="content">';
                           $html .='<label class="fs23 d-flex align-items-center" for="ta'.$type->id.'_'.$attr->id.'"><input class="pictype" style="margin: 5px; width: 25px; min-height: 25px;" type="radio" id="ta'.$type->id.'_'.$attr->id.'" name="pictures_type" data-price="'.$attr->price.'" value="'.$attr->title.'('.$attr->price.'KD)" '.$checked.'>'.$attr->title.'('.$attr->price.'KD)</label>';
